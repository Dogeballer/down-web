...
import store from '../../../InterfaceParam/store'
...
const InterfaceCallBackParamsTable = (props) => {
  ...
  const save = (form, id) => {
    form.validateFields((error, row) => {
      ...
      if (Math.sign(id) !== -1) {
        ...
        InterfaceAPI.updateInterfaceCallBackParams(id, row)
          .then((response) => {
            if (response.code === 0) {
        ...
        setLoading(true)
        row['return_interface'] = props.return_interface
        InterfaceAPI.createInterfaceCallBackParams(row)
          .then((response) => {
            if (response.code === 0) {
              props.onCallBackParamsChange()
              setEditingKey('')
  ...
  const handleDelFieldRow = (record) => {
    setLoading(true)
    InterfaceAPI.deleteInterfaceCallBackParams(record.id)
      .then((response) => {
        if (response.code === 0) {
          props.onCallBackParamsChange()
        }
